...
def skin_segment(img):
    #npImg = tf.make_ndarray(img)
    # input should be BGR image
    mask = skin_color_mask(img)

    kernel = np.ones((8, 8), np.uint8)
    closing = cv2.morphologyEx(np.float64(mask), cv2.MORPH_CLOSE, kernel)
    opening = cv2.morphologyEx(np.float64(closing), cv2.MORPH_OPEN, kernel)
    #tensorImg = opening.astype(np.uint8)[:, :, np.newaxis] * img
    return opening.astype(np.uint8)[:, :, np.newaxis] * img
    #return tf.convert_to_tensor(tensorImg, dtype=tf.float32)
...
def main():
    ...
    print(mat)
    """
    rgb_mask = skin_color_mask_from_RGB(rgb_norm_img)
    hsv_mask = skin_color_mask_from_HSV(hsv_img)
    ycrcb_mask = skin_color_mask_from_YCRCB(ycrcb_img)

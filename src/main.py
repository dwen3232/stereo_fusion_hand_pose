...
tf.compat.v1.app.flags.DEFINE_boolean("predict", False, "Running inference if true")
tf.compat.v1.app.flags.DEFINE_string(
    "input",
    "",
    "Input folder containing images")
tf.compat.v1.app.flags.DEFINE_string("model_dir", None, "Estimator model_dir")
tf.compat.v1.app.flags.DEFINE_string(
    "dset",
    "",
...
class Transformer(object):
  ...
  def unproject(self, xyz):
    ...
    def batch_matmul(a, b):
      return tf.reshape(
          tf.matmul(tf.reshape(a, [-1, a.shape.as_list()[2]]), b),
          [-1, a.shape.as_list()[1], a.shape.as_list()[2]])
...
def meshgrid(h):
  ...
  return tf.cast(ranx, dtype=tf.float32), tf.cast(rany, dtype=tf.float32)
...
def estimate_rotation(xyz0, xyz1, pconf, noise):
  ...
  xyz0 += tf.random.normal(tf.shape(input=xyz0), mean=0, stddev=noise)
  xyz1 += tf.random.normal(tf.shape(input=xyz1), mean=0, stddev=noise)

  pconf2 = tf.expand_dims(pconf, 2)
  cen0 = tf.reduce_sum(input_tensor=xyz0 * pconf2, axis=1, keepdims=True)
  cen1 = tf.reduce_sum(input_tensor=xyz1 * pconf2, axis=1, keepdims=True)
...
def relative_pose_loss(xyz0, xyz1, rot, pconf, noise):
  ...
  rotation = rot[:, :3, :3]
  frob_sqr = tf.reduce_sum(input_tensor=tf.square(r_transposed - rotation), axis=[1, 2])
  frob = tf.sqrt(frob_sqr)

  return tf.reduce_mean(input_tensor=frob_sqr), \
      2.0 * tf.reduce_mean(input_tensor=tf.asin(tf.minimum(1.0, frob / (2 * math.sqrt(2)))))
...
def separation_loss(xyz, delta):
  ...
  num_kp = tf.shape(input=xyz)[1]
  t1 = tf.tile(xyz, [1, num_kp, 1])

  t2 = tf.reshape(tf.tile(xyz, [1, 1, num_kp]), tf.shape(input=t1))
  diffsq = tf.square(t1 - t2)

  # -> [batch, num_kp ^ 2]
  lensqr = tf.reduce_sum(input_tensor=diffsq, axis=2)

  return (tf.reduce_sum(input_tensor=tf.maximum(-lensqr + delta, 0.0)) / tf.cast(
      num_kp * FLAGS.batch_size * 2, dtype=tf.float32))
...
def consistency_loss(uv0, uv1, pconf):
  ...
  wd = tf.reduce_sum(input_tensor=wd, axis=[1, 2])
  return tf.reduce_mean(input_tensor=wd)
...
def variance_loss(probmap, ranx, rany, uv):
  ...
  sh = tf.shape(input=ran)
  # [batch, num_kp, vh, vw, 2]
  ran = tf.reshape(ran, [1, 1, sh[0], sh[1], 2])

  sh = tf.shape(input=uv)
  uv = tf.reshape(uv, [sh[0], sh[1], 1, 1, 2])

  diff = tf.reduce_sum(input_tensor=tf.square(uv - ran), axis=4)
  diff *= probmap

  return tf.reduce_mean(input_tensor=tf.reduce_sum(input_tensor=diff, axis=[2, 3]))
...
def orientation_network(images, num_filters, is_training):
  ...
  with tf.compat.v1.variable_scope("OrientationNetwork"):
    ...
    sx = tf.reduce_sum(input_tensor=prob * ranx, axis=[2, 3])
    sy = tf.reduce_sum(input_tensor=prob * rany, axis=[2, 3])  # -> batch x modules
...
def keypoint_network(rgba,
  ...
  prob = tf.transpose(a=prob, perm=[0, 3, 1, 2])
  z = tf.transpose(a=z, perm=[0, 3, 1, 2])
  ...
  prob_viz = tf.expand_dims(tf.reduce_sum(input_tensor=prob, axis=1), 3)
  prob_vizs = [tf.expand_dims(prob[:, i, :, :], 3) for i in range(num_kp)]

  sx = tf.reduce_sum(input_tensor=prob * ranx, axis=[2, 3])
  sy = tf.reduce_sum(input_tensor=prob * rany, axis=[2, 3])  # -> batch x num_kp

  # [batch, num_kp]
  sill = tf.reduce_sum(input_tensor=prob * tf.expand_dims(mask, 1), axis=[2, 3])
  sill = tf.reduce_mean(input_tensor=-tf.math.log(sill + 1e-12))

  z = tf.reduce_sum(input_tensor=prob * z, axis=[2, 3])
...
def model_fn(features, labels, mode, hparams):
  ...
  for i in range(2):
    with tf.compat.v1.variable_scope("KeypointNetwork", reuse=i > 0):
      ...
      anneal = tf.cast(hparams["lr_anneal_end"] - tf.compat.v1.train.get_global_step(), dtype=tf.float32)
  ...
  with tf.compat.v1.variable_scope("output"):
    tf.compat.v1.summary.image("0_img0", touint8(features["img0"][:, :, :, :3]))
    tf.compat.v1.summary.image("1_combined", viz[0])
    for i in range(hparams["num_kp"]):
      tf.compat.v1.summary.image("2_f%02d" % i, vizs[0][i])
...
def predict(input_folder, hparams):
  ...
  for f in files:
    ...
    imageio.imwrite(os.path.join(output_folder, f), orig)
